-- phong Hub (PlayerGui): 8s loading, Add Brainrot, Duplicate v2 (progress 10s), Lucky Block panel riêng
...
local btnLucky=pill(body,"✨Lucky Block"); pillColor(btnLucky,114,106,240)

-- === Lucky Block Panel (riêng, kéo thả, có nút close)
local function ShowLuckyPanel()
    ...
    luckyInput.Text="set lucky block(ex:La Secret Combinasionas"
    ...
    luckyBtn.MouseButton1Click:Connect(function()
        print("🐇⏱️Set Brainrot in Lucky Block:", luckyInput.Text)
        -- TODO: Thay bằng logic thật (gửi sự kiện/HTTP/loadstring...) nếu bạn muốn
    end)
end
